...
def get_files(ann_dir, data_name, total_num=None):
...
def get_annotations(ann_dir, data_name, ids):
    assert COCO is not None, '`datasets.coco` requires `pycocotools`.'
    if data_name not in metas:
        metas[data_name] = COCO("%s/annotations/instances_%s.json" %
                                (ann_dir, data_name))
...
def evaluate(results,ann_dir, data_name, ovthresh=0.5, verbose=True):
